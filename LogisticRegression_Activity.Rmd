...
```{r}
#data=read.table("bank.txt",header = T,sep = ";")
...
library(caret)
data_row = createDataPartition(data$y, p = 0.7, list = F)
train_data <- data[data_row,]
test_data <- data[-data_row,]
str(test_data)
str(train_data)
...
```{r}
trained_data <- glm(y~., data = train_data, family = binomial)
```
...
library(ROCR)
library(gplots)
prob_train <- predict(trained_data,type="response")
#prob_train
...
perf_ormance <- performance(pred,measure = "tpr", x.measure = "fpr")
...
```{r}
plot(perf_ormance,col=rainbow(10),colorize=T, print.cutoffs.at=seq(0,1,0.05))
...
```{r}
perf_ormance_actual <- performance(pred,measure = "auc")

# Access the auc score from the performance object
auc <- perf_ormance_actual@y.values[[1]]
print(auc)
...
```{r}
prob_test <- predict(trained_data,test_data,type = "response")
